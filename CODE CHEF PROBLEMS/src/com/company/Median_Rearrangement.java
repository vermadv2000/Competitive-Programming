...
class Median_Rearrangement {
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            long left = n/2L, right = left*n, ans = -1L, y = left;

            if (is_k_possible(arr,right,y,k,n)) {
                out.println(arr[(int)right]);
                continue;
            }
            ...
            while (left <= right) {
                ...
                if (is_k_possible(arr,mid,y,k,n)) {
                    ans = Math.max(ans,arr[(int)mid]);
                    left = mid + 1;
                }
                else right = mid - 1;
            }
    ...
    static boolean is_k_possible (long [] arr, long x, long y, long k, int n){
    ...
    static class FastReader {
        ...
        {
            byte[] buf = new byte[128000]; // line length
            int cnt = 0, c;
            while ((c = read()) != -1) {
                if (c == '\n') {
                    if (cnt != 0) {
