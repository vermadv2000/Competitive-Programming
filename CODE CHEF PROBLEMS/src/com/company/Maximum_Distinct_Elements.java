...
class Maximum_Distinct_Elements {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (x.containsKey(z)) x.put(z,x.get(z)+i+" ");
                else x.put(z,i+" ");
            }
            ...
            for (int j = 0; j < n; j++) {
                int y = arr[j];
                String [] s = x.get(y).trim().split(" ");
                for (int i = 0; i < s.length; i++) {
                    if (count > y-1) {
                        ans[Integer.parseInt(s[i])] = y;
                        if (count == 0) count++;
                    }
                    else {
                        ans[Integer.parseInt(s[i])] = count;
                        count++;
                    }
                }
                j += s.length-1;
