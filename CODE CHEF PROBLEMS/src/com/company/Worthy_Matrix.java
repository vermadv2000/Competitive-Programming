...
public class Worthy_Matrix {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0){
            ...
            // TAKING ELEMENT
            long element, temp, count;
            for(int i=1; i<=n; i++){
                temp = 0;
                for(int j=1; j<=m; j++){
                    element = sc.nextLong();
                    arr[i][j] = arr[i-1][j] + element + temp;
                    temp += element;
                }
            }
            // FINDING WORTHY MATRIX
            count = 0;
            int order = 1, check;
            while(order<=n) {
                for (int i=order; i<=n; i++) {
                    double sum1 = arr[i][m] + arr[i-order][m-order] - arr[i][m-order] - arr[i-order][m];
                    check = 0;
                    if (sum1/(order*order) >= k) {
                        count++;
                        for (int j=1; j<=m-order; j++) {
                            double sum2 = arr[i][m-j] + arr[i-order][m-order-j] - arr[i][m-order-j] - arr[i-order][m-j];
                            if(sum2/(order*order)<k) break;
                            count++;
                            check = j;
                        }
                    }
                    if(check == m-order) {
                        count += (long) (m - order + 1) *(n-i);
                        break;
                    }
                }
                order++;
