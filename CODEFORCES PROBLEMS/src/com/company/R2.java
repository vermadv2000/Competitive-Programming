...
public class R2 {
    ...
    void solve () throws IOException {
        ...
        Triplet[] x = new Triplet[n];
        for (int i = 0; i < n; i++) {
            int k = nextInt();

            x[i] = new Triplet(arr[i],k,i);
        }

        Arrays.sort(x);

        Pair[] parent = new Pair[n];
        for (int i = 0; i < n; i++) parent[i] = new Pair(i,0);
        ...
        for (int i = 0; i < n; i++) out.println(ans[n-i-1]);
    }

//    long union (int i, int j, Pair[] p, int[] r) {
//
//    }
