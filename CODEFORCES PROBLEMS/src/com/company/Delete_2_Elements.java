...
public class Delete_2_Elements {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (x.containsKey(e)) x.put(e,x.get(e)+1);
                else x.put(e,1);
            }
            ...
            long k = (long) sum, pairs = 0;
            for (int i = 0; i < n; i++) {
                if (x.containsKey(k - arr[i])) {
                    if (k == 2*arr[i]) {
                        int temp = x.get(arr[i]);
                        pairs += ((long) temp *(temp-1))/2;
                        x.remove(arr[i]);
                    }
                    else {
                        pairs += (long) x.get(arr[i]) *x.get(k - arr[i]);
