...
class Towe {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (x.higherKey(k) != null) {
                int temp = x.higherKey(k);

                if (x.get(temp) == 1) x.remove(temp);
                else x.put(temp, x.get(temp) - 1);
            }

            if (x.containsKey(k)) x.put(k,x.get(k)+1);
            else x.put(k,1);
        }

        long sum = 0;
        for (long z: x.values()) sum += z;
        out.println(sum);
