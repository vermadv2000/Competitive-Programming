...
class Sub2 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n+1; i++) {
            long z = sum[i];
            if (x.containsKey(z)) x.put(z,x.get(z)+1);
            else x.put(z,1);
        }
        ...
        long ans = 0;
        for (int i = n; i >= 0; i--) {
            if (x.containsKey(sum[i] - k)) ans += x.get(sum[i] - k);

            if (x.get(sum[i]) == 1) x.remove(sum[i]);
            else x.put(sum[i],x.get(sum[i]) - 1);
        }

        out.println((k == 0)?ans-n-1:ans);
