...
class DRSQ {
    ...
    static class Segment_Tree {
        ...
        public Segment_Tree (int n) {
            ...
            st = new long[2*size - 1];
        }

        public void build (long[] arr) {
            st[0] = build_util(arr,0,arr.length-1,0);
        }
        public long build_util(long[] arr, int l, int r, int curr) {

            if (l == r) return st[curr] = arr[l];

            int m = mid(l,r);
            return st[curr] = build_util(arr, l, m, 2*curr + 1) + build_util(arr, m+1, r, 2*curr+2);
        }
        ...
        public long query_util (int l, int r, int curr) {

            if (l >= x && r <= y) return st[curr];
            else if (l > y || r < x) return 0;
            else {
                int m = mid(l,r);
                return query_util(l,m,2*curr+1) + query_util(m+1,r,2*curr+2);
            }
        }
        ...
        void print(){
            for (long z: st) System.out.print(z+" ");
            System.out.println();
        }
    }
