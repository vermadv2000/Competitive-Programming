...
class Coun {
    public static void main(String[] args) throws IOException {
        ...
        out.println(count_set_bits(sc.nextLong()));

        sc.close();
        out.close();
    }
    static long count_set_bits (long a){

        if (a<=0) return a;

        //int mod = 1000000007;
        long n = (long)(Math.log(a)/Math.log(2));

        long count = (a + 1 - (long)Math.pow(2,n)) + (n * (long)Math.pow(2,n-1));
        return (count + count_set_bits((a - (long)Math.pow(2,n))));
    }
