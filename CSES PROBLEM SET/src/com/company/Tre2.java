...
class Tre2 {
    ...
    static void dfs (int i, int p) {
        if (i != 0) {
            ans[i] = ans[p] + adj.length - 2L*child[i];
        }
    ...
    static long[] dis;
    static long dis_util (int i, int p) {
        long ans = 0;
        for (int z: adj[i]) {
            if (z != p) {
                ans += dis_util(z,i) + child[z];
            }
        }
    ...
    static int[] child;
    static int child_util(int i, int p) {
        int ans = 0;
        for (int z: adj[i]) {
            if (z != p) {
                ans += child_util(z,i);
